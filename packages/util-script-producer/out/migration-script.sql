...
--activation_code_list_view
CREATE VIEW activation_code_list_view
AS
WITH 
base_query_cte AS
(
...
--current_user_course_bridge_view
CREATE VIEW current_user_course_bridge_view
AS
SELECT
    ucb.* 
FROM 
...
--question_module_compare_view
CREATE VIEW question_module_compare_view
AS
WITH
flat_cte AS 
(
...
SELECT 
	...
	cd.is_precourse_survey_required,
...
--latest_exam_result_view
CREATE VIEW latest_exam_result_view
AS
SELECT
    erv.*
FROM public.latest_answer_session_view lasv
...
SELECT 
    lasv.user_id,
    lev.course_id,
    MAX(esv.score_percentage) final_exam_score_percentage
FROM public.latest_answer_session_view lasv
...
-- CREATE constraints
-- activation_code_uniqe_constraint 
ALTER TABLE activation_code
ADD CONSTRAINT activation_code_uniqe_constraint
UNIQUE (code);
-- coin_transaction_valid_relation_enforce_constraint 
...
-- course_access_bridge_constraints 
-- UNIQUE COMPANY 
...
-- permission_assignment_bridge_constraints 
...
-- prequiz_completion_constraints 
...
-- role_permission_bridge_constraint 
