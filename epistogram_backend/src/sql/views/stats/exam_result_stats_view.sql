...
FROM max_score_answer_session msas

LEFT JOIN public.answer_session_view asv
ON asv.answer_session_id = msas.answer_session_id

LEFT JOIN public.exam_version ev
ON ev.id = msas.exam_version_id
...
LEFT JOIN public.user u
ON u.id = msas.user_id

LEFT JOIN answer_session_lengths asl
ON asl.answer_session_id = msas.answer_session_id
...
ON fcaq.answer_session_id = msas.answer_session_id

LEFT JOIN question_count qc
ON qc.exam_version_id = msas.exam_version_id

LEFT JOIN public.exam_score_view esv
ON esv.answer_session_id = msas.answer_session_id
