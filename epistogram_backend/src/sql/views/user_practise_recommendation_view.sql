WITH user_answers AS (
    SELECT ga.question_id,
        ...
        ROW_NUMBER() OVER(
            PARTITION BY ga.question_id
            ORDER BY ga.creation_date DESC
        ) AS order_in_group
    FROM public.given_answer ga
        LEFT JOIN public.answer_session AS ase ON ase.id = ga.answer_session_id
        LEFT JOIN public.question AS q ON q.id = ga.question_id
        LEFT JOIN public.video AS v ON v.id = q.video_id
    ORDER BY ga.question_id,
        ga.creation_date,
        ase.video_id
)
SELECT question_id,
    ...
    COALESCE(
        SUM(NULLIF(is_correct::int, 0)) / SUM(NULLIF(given_answer_count, 0)),
        0
