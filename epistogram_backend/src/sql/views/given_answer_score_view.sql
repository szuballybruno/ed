...
(
	SELECT 
		...
		SUBSTRING(qd.question_text, 1, 15) question_text,
...
given_answer_score_calc_cte AS (
	SELECT 
		gasc.user_id,	
		gasc.answer_session_id,
...
(
	SELECT
		...
		CASE WHEN sq.user_id IS NOT NULL 
			...
			ELSE 0
		END score
	FROM given_answer_score_calc_cte sq
